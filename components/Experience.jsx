export default function ExperienceComponent() {
  return (
    <>
      <h3 className="font-light tracking-widest text-3xl col-span-3 mb-10">
        EXPERIENCE.
      </h3>

      <ul className="bg-black outline-none list-none ">
        <li className="p-8 grid grid-cols-5 gap-5">
          <div className=" "></div>
          <div className="col-span-4 relative pl-4">
            <h6 className="font-bold text-lg">Software Developer</h6>
            <p className="font-extralight text-xs mt-1">
              Corporate IT Solution Co.Ltd | Remote | Nov 2022 - Present
            </p>
            <p className="font-extralight text-sm text-justify w-4/5  mt-5">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut
              enim ad minim veniam, quis nostrud exercitation ullamco laboris
              nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor.
            </p>
            <span className="px-2 py-1 text-xs tracking-wider bg-indigo-500 font-light shadow-md shadow-indigo-700 absolute top-0 right-0">
              FULL-TIME
            </span>
          </div>
        </li>
        <hr className="opacity-20" />
        <li className="p-8 grid grid-cols-5 gap-5">
          <div className=" "></div>
          <div className="col-span-4 relative pl-4">
            <h6 className="font-bold text-lg">Software Developer</h6>
            <p className="font-extralight text-xs mt-1">
              Corporate IT Solution Co.Ltd | Remote | Nov 2022 - Present
            </p>
            <p className="font-extralight text-sm text-justify w-4/5  mt-5">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut
              enim ad minim veniam, quis nostrud exercitation ullamco laboris
              nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor.
            </p>
            <span className="px-2 py-1 text-xs tracking-wider bg-indigo-500 font-light shadow-md shadow-indigo-700 absolute top-0 right-0">
              FULL-TIME
            </span>
          </div>
        </li>
        <hr className="opacity-20" />
        <li className="p-8 grid grid-cols-5 gap-5">
          <div className=" "></div>
          <div className="col-span-4 relative pl-4">
            <h6 className="font-bold text-lg">Software Developer</h6>
            <p className="font-extralight text-xs mt-1">
              Corporate IT Solution Co.Ltd | Remote | Nov 2022 - Present
            </p>
            <p className="font-extralight text-sm text-justify w-4/5  mt-5">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut
              enim ad minim veniam, quis nostrud exercitation ullamco laboris
              nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor.
            </p>
            <span className="px-2 py-1 text-xs tracking-wider bg-indigo-500 font-light shadow-md shadow-indigo-700 absolute top-0 right-0">
              FULL-TIME
            </span>
          </div>
        </li>
        <hr className="opacity-20" />
        <li className="p-8 grid grid-cols-5 gap-5">
          <div className=" "></div>
          <div className="col-span-4 relative pl-4">
            <h6 className="font-bold text-lg">Software Developer</h6>
            <p className="font-extralight text-xs mt-1">
              Corporate IT Solution Co.Ltd | Remote | Nov 2022 - Present
            </p>
            <p className="font-extralight text-sm text-justify w-4/5  mt-5">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut
              enim ad minim veniam, quis nostrud exercitation ullamco laboris
              nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor.
            </p>
            <span className="px-2 py-1 text-xs tracking-wider bg-indigo-500 font-light shadow-md shadow-indigo-700 absolute top-0 right-0">
              FULL-TIME
            </span>
          </div>
        </li>
        <hr className="opacity-20" />
      </ul>
    </>
  );
}
